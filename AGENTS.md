...
- **Avoid `force`, hover-only setup, and positional locators (`first`/`last`/`nth`) unless the behavior under test is specifically about that interaction model.** Exception: for buttons/checkboxes inside hover-gated CSS containers (`pointer-events: none` / `opacity: 0` on an ancestor), use `evaluate((el: HTMLElement) => el.click())` — this fires a **trusted** native click (`isTrusted: true`) that Svelte 5 always handles, and is a programmatic API call that bypasses both CSS `pointer-events` and Playwright's `checkOpacity` actionability check. **Do NOT use** `{ force: true }` (native mouse events still respect pointer-events on parents) or `dispatchEvent(new MouseEvent(...))` (creates untrusted events that Svelte 5 may filter).
